"""
Main training script for LLM-CLP (Counterfactual Logit Pairing).
...
from src.llm_clp.data.dataset import get_causal_fair_loader
from src.llm_clp.models.losses import CounterfactualLogitPairing, CounterfactualSupConLoss
from src.llm_clp.models.classifier import DebertaV3CausalFair
from src.llm_clp.evaluation.metrics import evaluate_causal_fairness
...
def train_one_epoch(
    ...
    Loss: L_CE(x, y) + λ_clp * L_CLP(x, x_cf) + λ_con * L_SupCon(z, z_cf, y)
    """
    model.train()
...
def main():
    parser = argparse.ArgumentParser(
        description="LLM-CLP Training: Counterfactual Logit Pairing",
    ...
    parser.add_argument("--cf_path", type=str, default=None,
                        help="Path to counterfactual parquet file")
    ...
    if args.cf_path and os.path.exists(args.cf_path):
        ...
        print(f"[CF] Loaded {len(cf_df)} counterfactual pairs")
    else:
        print("[CF] No counterfactual data provided, training without CLP")
    ...
    print(f"[Model] {args.model_name} | {sum(p.numel() for p in model.parameters()):,} params")
