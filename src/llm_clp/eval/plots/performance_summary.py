...
from src.llm_clp.common.paths import eval_path, EVAL_DIR, plot_path
...
def plot_performance_summary(df):
    ...
    plt.title('性能对比：平均偏差 AUC（越高越好）', fontsize=14, fontweight='bold')
    plt.xlim(0.8, 1.0)  # 假设都在这个区间
    plt.xlabel('平均偏差 AUC 分数')
    ...
    plt.tight_layout()
    output_path = plot_path("model_performance_summary.png")
