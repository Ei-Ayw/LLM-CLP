...
from src.common.paths import model_path, log_path
...
class DebertaV3GetFair(nn.Module):
    """DeBERTa-V3，附带梯度公平约束
    使用标准 forward 推理，通过 embedding hook 捕获梯度"""
    ...
    def forward(self, input_ids, attention_mask, capture_embed_grad=False):
        ...
        if capture_embed_grad:
            hook.remove()
            # 对 embedding 输出注册梯度 hook，用于捕获反向传播时的梯度
...
def train_one_epoch(model, loader, optimizer, scheduler, device, args, tokenizer):
    """GetFair 训练：使用 torch.autograd.grad + create_graph=True 实现二阶梯度惩罚"""
    model.train()
    ...
    for i, batch in enumerate(pbar):
        ...
        if args.lambda_gf > 0 and embed_out is not None and embed_out.requires_grad:
            # 用 autograd.grad 计算 CE 对 embedding 输出的梯度，并保留计算图
        ...
        # 总损失 = CE + lambda * 梯度公平惩罚，统一执行 backward
...
def main():
    ...
    best_f1 = 0
    save_path = model_path("{}.pth".format(exp_name))
    ...
    results_path = log_path("{}_results.json".format(exp_name))
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)
    print("结果保存至: {}".format(results_path))
    print("\n>>> 下一步: python -m src.eval.eval_causal_fairness --checkpoint {} --dataset {}".format(
