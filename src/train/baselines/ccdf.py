...
def train_bias_model(bias_model, loader, device, tokenizer, epochs=5, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(loader, desc=f"[Bias Epoch {epoch+1}]"):
            ...
            n += 1
    print(f"  偏置模型 Epoch {epoch+1}: loss={total_loss/n:.4f}")

    bias_model.eval()
    print("[阶段 1] bias 模型训练完成。")
...
def train_one_epoch(model, bias_model, loader, optimizer, scheduler, scaler, device, args, tokenizer):
    ...
    for i, batch in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast():
            ...
            # TDE：主模型 logits 减去 bias 模型 logits
...
def main():
    ...
    # 阶段 2：训练主模型（使用 TDE 去偏）
    model = DebertaV3CCDF(args.model_name, num_classes=2).to(device)
    print(f"\n[阶段 2] 主模型训练 | lambda_kl={args.lambda_kl} tde_alpha={args.tde_alpha}")
    ...
    print(f"\n测试集（TDE）: F1={test_metrics['macro_f1']:.4f} AUC={test_metrics['auc_roc']:.4f}")
