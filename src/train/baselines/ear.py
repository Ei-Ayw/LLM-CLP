...
class DebertaV3EAR(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, return_attentions=False):
        ...
        if return_attentions and outputs.attentions is not None:
            result["attentions"] = outputs.attentions  # tuple of (B, H, L, L)，各层注意力权重
        return result
...
def main():
    ...
    print(f"[模型] EAR | lambda_ear={args.lambda_ear}")
    ...
    print(f"\n测试集: F1={test_metrics['macro_f1']:.4f} AUC={test_metrics['auc_roc']:.4f}")
    ...
    print(f"\n>>> 下一步: python -m src.eval.eval_causal_fairness --checkpoint {save_path} --dataset {args.dataset}")
