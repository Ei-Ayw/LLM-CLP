...
毒性分类器会学习到**身份词与毒性标签之间的虚假相关性**——例如将 "Muslims" 当作毒性信号。这导致三个核心问题：

1. **虚假特征依赖**：模型把身份词当作判断毒性的主要依据（spurious feature），而非真实的有毒内容
2. **群体偏见**：对某些群体（如 LGBTQ+、穆斯林、黑人）更敏感，导致过度预测毒性
3. **违反因果公平性**：仅改变身份群体不应改变毒性预测，但 baseline 模型会翻转预测

**CFR 指标说明：**

CFR (Counterfactual Flip Rate) 衡量模型对身份词的敏感性：

```
CFR = 翻转的样本数 / 总样本数
```

其中"翻转"指原始样本和反事实样本的预测不一致（如 toxic → non-toxic）。

- **理想情况**：CFR = 0（完美公平，模型完全忽略身份词，只关注真实有毒内容）
- **HateXplain 测试集**：
  - E1 Baseline: CFR = 0.1568 (15.68% 样本翻转)
  - E3 LLM+CLP: CFR = 0.0563 (5.63% 样本翻转)
  - **降低 64%**
- **Case Study 精选案例**：
  - E1 Baseline: CFR = 1.0 (100% 翻转) — 专门挑选 E1 翻转的案例
  - E3 LLM+CLP: CFR = 0.0 (0% 翻转) — 证明 E3 完全解决了这些偏见案例

传统的反事实数据增强 (CDA) 使用简单的词替换（如 "Muslim" → "Christian"），产生语法生硬、文化不合理的文本。我们利用大模型生成**语义自然**的反事实样本，进行文化适配的替换（如 "mosque" → "church", "hijab" → "cross necklace"），然后用因果公平目标训练。
