...
# =============================================================================
# ### 训练脚本：train_deberta_v3_cls_only.py ###
# 设计说明：
# 针对 Ablation-1 的物理对齐脚本。
# 使用 model_deberta_v3_cls_only.py 中的架构，仅使用 CLS 池化层。
# =============================================================================
...
def train_one_epoch(model, loader, optimizer, scheduler, device, accum_steps):
    ...
    total_loss = 0
    pbar = tqdm(loader, desc="[Train Ablation CLS]")
...
def main():
    ...
    save_path = os.path.join(BASE_DIR, "src_result", f"DebertaV3_CLSOnly_Sample{args.sample_size}_{datetime.now().strftime('%m%d_%H%M')}.pth")
    ...
    model = DebertaV3CLSOnly().to(device)
