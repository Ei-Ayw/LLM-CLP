...
# =============================================================================
# ### 训练脚本：train_vanilla_transformers.py ###
# 设计说明：
# 这是一个通用的原生 Transformer 训练脚本，支持从本地加载 BERT, RoBERTa, DeBERTa。
# 用于快速产生 Group 3 (Transformer Baselines) 的所有结果。
# =============================================================================
...
def train_one_epoch(model, loader, optimizer, scheduler, device, accum_steps):
    ...
    for i, batch in enumerate(pbar):
        ...
        # 注意：RoBERTa 不支持 token_type_ids，VanillaTransformer 内部 forward 已处理
...
def main():
    parser = argparse.ArgumentParser(description="Vanilla Transformer Training")
    parser.add_argument("--model_path", type=str, required=True, help="本地预训练权重路径")
    parser.add_argument("--model_tag", type=str, required=True, help="模型标识，如 RoBERTa_Base")
    ...
    save_name = f"Vanilla_{args.model_tag}_Sample{args.sample_size}_{timestamp}.pth"
    save_path = os.path.join(BASE_DIR, "src_result", save_name)

    print(f"\n>>> 启动原生 {args.model_tag} 实验: {save_name}")
    ...
    # 模型初始化
    model = VanillaTransformer(args.model_path).to(device)
    ...
    for epoch in range(args.epochs):
        train_one_epoch(model, train_loader, optimizer, scheduler, device, accum_steps=1)
