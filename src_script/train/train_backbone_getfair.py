"""
=============================================================================
Backbone GetFair — 训练 GetFair 基线 (BERT / RoBERTa / DeBERTa-v3)
...
from model_backbone_baselines import BackboneGetFair, build_identity_mask
...
def train_one_epoch(model, loader, optimizer, scheduler, device,
                    criterion, tokenizer, lambda_gf, grad_accum=2):
    model.train()
    ...
    for i, batch in enumerate(pbar):
        ...
        if lambda_gf > 0 and embed_out is not None and embed_out.requires_grad:
            ...
            if id_mask.sum() > 0:
                grad_norm = (embed_grad ** 2).sum(dim=-1)
                l_gf = (grad_norm * id_mask).sum() / (id_mask.sum() + 1e-9)

        loss = l_ce + lambda_gf * l_gf
        ...
        n += 1
        pbar.set_postfix(loss=f"{total_loss/n:.4f}", gf=f"{total_gf/n:.4f}")
...
def main():
    ...
    print("[GPU] GetFair uses single GPU (create_graph=True incompatible with DataParallel)")

    exp_name = f"GetFair_{args.backbone}_{args.dataset}_seed{args.seed}_{datetime.now().strftime('%m%d_%H%M')}"
    ...
    model     = BackboneGetFair(model_path, backbone_type=args.backbone, num_classes=2).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        tm = train_one_epoch(model, train_loader, optimizer, scheduler, device,
                             criterion, tokenizer, args.lambda_gf, args.grad_accum)
        vm = evaluate(model, val_loader, device)
        print(f"  Train loss={tm['loss']:.4f} CE={tm['ce']:.4f} GF={tm['gf']:.4f}")
