"""
=============================================================================
Backbone EAR — 训练 EAR 基线 (BERT / RoBERTa / DeBERTa-v3)
EAR: Entropy-based Attention Regularisation
L_total = L_CE + λ_ear * L_AttentionEntropy
=============================================================================
"""
...
from model_backbone_baselines import BackboneEAR, attention_entropy_loss
...
def train_one_epoch(model, loader, optimizer, scheduler, scaler, device,
                    criterion, lambda_ear, grad_accum=2):
    model.train()
    ...
    for i, batch in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast():
            ...
            l_ear = attention_entropy_loss(out['attentions'], mask) if 'attentions' in out else torch.tensor(0.)
            loss  = l_ce + lambda_ear * l_ear
        ...
        total_ear  += l_ear.item() if isinstance(l_ear, torch.Tensor) else l_ear
        n += 1
        pbar.set_postfix(loss=f"{total_loss/n:.4f}", ear=f"{total_ear/n:.4f}")
...
def main():
    ...
    exp_name = f"EAR_{args.backbone}_{args.dataset}_seed{args.seed}_{datetime.now().strftime('%m%d_%H%M')}"
    ...
    model     = BackboneEAR(model_path, backbone_type=args.backbone, num_classes=2).to(device)
    if n_gpu > 1:
    ...
    for epoch in range(args.epochs):
        ...
        tm = train_one_epoch(model, train_loader, optimizer, scheduler, scaler,
                             device, criterion, args.lambda_ear, args.grad_accum)
        vm = evaluate(model, val_loader, device)
        print(f"  Train loss={tm['loss']:.4f} CE={tm['ce']:.4f} EAR={tm['ear']:.4f}")
