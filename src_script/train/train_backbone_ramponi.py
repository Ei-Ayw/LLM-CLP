"""
=============================================================================
Backbone Ramponi — 训练 Ramponi 基线 (BERT / RoBERTa / DeBERTa-v3)
Ramponi: Adversarial Debiasing via Gradient Reversal Layer (GRL)
L_total = L_CE + λ_adv * L_adversarial
=============================================================================
"""
...
from model_backbone_baselines import BackboneRamponi
...
class IdentityTextDataset(Dataset):
    def __init__(self, df, tokenizer, max_len=128):
        ...
        if 'has_identity' in df.columns:
            self.id_labels = df['has_identity'].astype(int).values
        else:
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'identity_label': torch.tensor(self.id_labels[idx],   dtype=torch.long),
        }
...
def train_one_epoch(model, loader, optimizer, scheduler, scaler, device,
                    criterion, lambda_adv, epoch, total_epochs, grad_accum=2):
    model.train()
    ...
    for i, batch in enumerate(pbar):
        ...
        id_lbl   = batch['identity_label'].to(device)

        with torch.cuda.amp.autocast():
            out   = model(ids, mask, alpha=alpha)
            l_ce  = criterion(out['logits'], lbl)
            l_adv = F.cross_entropy(out['adv_logits'], id_lbl)
        ...
        n += 1
        pbar.set_postfix(loss=f"{total_loss/n:.4f}", adv=f"{total_adv/n:.4f}",
                         alpha=f"{alpha:.3f}")
...
def main():
    ...
    exp_name = f"Ramponi_{args.backbone}_{args.dataset}_seed{args.seed}_{datetime.now().strftime('%m%d_%H%M')}"
    ...
    model     = BackboneRamponi(model_path, backbone_type=args.backbone,
    ...
    for epoch in range(args.epochs):
        ...
        tm = train_one_epoch(model, train_loader, optimizer, scheduler, scaler,
                             device, criterion, args.lambda_adv, epoch, args.epochs,
                             args.grad_accum)
        vm = evaluate(model, val_loader, device)
        print(f"  Train loss={tm['loss']:.4f} CE={tm['ce']:.4f} ADV={tm['adv']:.4f} α={tm['alpha']:.3f}")
