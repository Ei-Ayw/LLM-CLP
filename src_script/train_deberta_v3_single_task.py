...
# =============================================================================
# ### 训练脚本：train_deberta_v3_single_task.py ###
# 设计说明：
# 针对 Ablation-2 的物理对齐脚本。
# 使用 model_deberta_v3_single_task.py 中的架构，仅训练毒性分类单任务。
# =============================================================================
...
def train_one_epoch(model, loader, optimizer, scheduler, device, accum_steps):
    ...
    total_loss = 0
    pbar = tqdm(loader, desc="[Train SingleTask]")
    for i, batch in enumerate(pbar):
        ids, mask = batch['input_ids'].to(device), batch['attention_mask'].to(device)
        y_tox = batch['y_tox'].to(device).unsqueeze(-1)
        out = model(ids, mask)
        loss = criterion(out['logits_tox'], y_tox) / accum_steps
        loss.backward()
...
def main():
    ...
    save_path = os.path.join(BASE_DIR, "src_result", f"DebertaV3_SingleTask_Sample{args.sample_size}_{datetime.now().strftime('%m%d_%H%M')}.pth")
    ...
    model = DebertaV3SingleTask().to(device)
